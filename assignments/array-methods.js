...
let fullName = [];
runners.forEach(fullNameCallback =>{
    return fullName.push(`Runner: ${fullNameCallback.first_name} ${fullNameCallback.last_name}`);  
            // forEach does NOT create a new array automatically. You need to make your own array, hence the .push!!!
});
...
let allCaps = [];
allCaps = runners.map(allCapsCallback=>{
    return allCapsCallback.first_name.toUpperCase();
});
console.log(allCaps); 
...
let largeShirts = [];
largeShirts = runners.filter(shirtCallback=>{
    return shirtCallback.shirt_size === "L";
});
console.log(largeShirts);  //largeShirts.first_name doesn't work because you would have to iterate through each object.

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
ticketPriceTotal = runners.reduce((total,ticketCallback)=>{
    return total += ticketCallback.donation;
},0);
